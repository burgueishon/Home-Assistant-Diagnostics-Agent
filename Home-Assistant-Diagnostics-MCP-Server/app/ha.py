...
async def get_client() -> httpx.AsyncClient:
    """Get a persistent httpx client for Home Assistant API calls, safe across event loops."""
    global _client, _client_loop
    loop = asyncio.get_event_loop()

    # Recreate client if none, closed, or loop changed
    needs_new = (
        _client is None or
        getattr(_client, "is_closed", False) or
        _client_loop is None or
        _client_loop is not loop
    )

    if needs_new:
        if _client is not None and hasattr(_client, "aclose") and not getattr(_client, "is_closed", False):
            try:
